        ...
        return (
            <div className="container my-3">
               {this.state.getuser && <button className="btn btn-success btn-lg float-end" type="submit" onClick={this.handleGetUserClick}>Get User</button>}
                {this.state.next && <div><h1 className="text-center" style={{color: "white"}}>Style Stars Users</h1><hr  style={{color: "white"}} /></div>}
              ...
              { this.state.buttons && <div className="container d-flex justify-content-between">
                    <button disabled={this.state.page <= 1} type="button" className="btn btn-lg btn-danger" onClick={this.handlePrevClick}>&larr; Previous</button>
                    <button disabled={this.state.page === this.state.total_pages} type="button" className="btn btn-lg btn-danger" onClick={this.handleNextClick}>Next &rarr;</button>
                </div> }
            </div>
        )
    }
